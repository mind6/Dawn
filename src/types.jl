...
"""
TradeRunContext manages a complete trading run context with all its state.

This consolidates all aspects of a trade run:
- Core run information and strategy data
- Provider controls and execution state
- Selection state (current trade, date, etc.)
- Summary data and analysis results
"""
mutable struct TradeRunContext
	...
	runtsks::Union{Nothing, Vector{Task}}
	
	# Selection state
	curtradectrl::Union{Nothing, TradeProviderControl}
	curtradeidx::Int
	curdate::Union{Nothing, UnixDate}
	curbday::Union{Nothing, AbstractDataFrame}
	
	# Summary state
	tradesummary::Union{Nothing, AbstractDataFrame}
	tradesummary_gb::Union{Nothing, GroupedDataFrame}
	monthsummary::Union{Nothing, AbstractDataFrame}
	monthsummary_gb::Union{Nothing, GroupedDataFrame}
	monthsummary_combined::Union{Nothing, AbstractDataFrame}
	
	function TradeRunContext(run_name, run)
		new(
			# Core properties
			Dates.now(), nothing, nothing, run_name, 
			# Strategy implementation
			run,
			# Provider controls
			TradeProviderControl[], Dict{Symbol,TradeProviderControl}(), nothing,
			# Selection state
